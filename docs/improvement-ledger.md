...
| Shared references | **14** | `plugins/proofpunk/references/*.md` (the work-order's 13 omitted `run-trace-schema.md`) |
| Commands (Claude Code) | **6** | `plugins/proofpunk/commands/*.md` |
| Commands (OpenCode) | **6** | `plugins/proofpunk/opencode/commands/*.md` |
| Hook scripts | **9** | `plugins/proofpunk/hooks/*.sh` |
| Hook event keys | **7** | top-level keys of `hooks.json` `.hooks` |
| Hook registrations | **11** | every `command` object under those events |
...
| 16 | **CLOSED this lane** | Before: `run-trace-schema.md` shipped, cited by no `SKILL.md`. After: `plugins/proofpunk/skills/proofpunk/SKILL.md:99` and `end-user-testing/SKILL.md:133-135`. `python3 tools/verify-router-links.py` rc=0, `doctrine_refs=14 all_resolved=true` | 3 |
...
| I5+I6 run-trace-schema cite | `proofpunk/SKILL.md:99`, `end-user-testing/SKILL.md:133-135` | `step-04-…` `shipped-not-active: run-trace-schema.md` | `step-06-router-links-after-cite.log` rc=0 `doctrine_refs=14 all_resolved=true`; `step-10-…` `refs disk=14 cited=14` | **PASS** (script-level) |
...
- **Shared-reference count is 14**, not the work-order's 13. `run-trace-schema.md` is the extra file. Class 2 (sibling) must accept 14 or it will FAIL live docs that are now correct.
