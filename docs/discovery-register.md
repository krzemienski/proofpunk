...
| D6 | Ground-truth counts | 18 skills · 15 references · 6+6 commands · 10 hook files · **7 event keys / 12 registrations / 10 distinct scripts** · 3/4/3 agents · 22 tools. | `d6-d7-measurement.md` | **high** |
...
4. **A wrong resolver produced a false 183/296-unresolved reading** — citations must resolve relative to the citing file. All 15 doctrine references are fine.
5. **`grep` found no matches ≠ absence** — `test-hooks.sh` resolves scripts through `$HOOKS`, so a literal-string count reported 0 for a harness that invokes all 10.
