...
Execution-first delivery plugin for Claude Code, oh-my-pi (OMP), and OpenCode: 19 skills that make "done" mean *proven by end-user testing*. The AI drives the real system as an end user, and any claim it did not actually execute is reported UNVERIFIED, never PASS. No mocks, no stubs, no test-mode bypasses.
