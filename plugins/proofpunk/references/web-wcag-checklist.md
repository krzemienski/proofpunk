...
### APCA — the modern lightness-aware metric

WCAG 2.x ratios collapse to 1 number that ignores the eye's non-linear response to lightness. APCA (used in WCAG 3 draft) returns a signed `Lc` value tuned to perceptual contrast. For body text, **`|Lc| ≥ 60`** is the working minimum; UI components use 45-60 depending on size.

Worked example (drawn from iter-16 finding F-1D):

```
Foreground: #777   (caption-muted)
Background: #222   (dark mode body)
APCA Lc:    ~32    (FAIL — needs ≥ 60 for body text)
WCAG 2 ratio: ~3.9 (would pass AA-Large, but the APCA signal is correct)
```

Recipe to compute live:

```bash
# Capture computed colors via agent-browser
agent-browser open <url>
agent-browser eval "getComputedStyle(document.querySelector('<sel>')).color + ' on ' + getComputedStyle(document.body).backgroundColor"
# Output: "rgb(119, 119, 119) on rgb(34, 34, 34)"

# Pipe through any APCA implementation (apca-w3 npm pkg, or python-apca-rs)
# Confirm |Lc| ≥ 60 for body text, ≥ 45 for ≥18pt UI text
```

When the WCAG 2 ratio passes but APCA fails (or vice versa), prefer APCA's verdict — it correlates better with user-reported readability complaints.
