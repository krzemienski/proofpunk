> Incorporated from the `claude-code-analyzer` skill (skills-ref.zip). **Adaptation:** usage analytics companion — it answers "what did the tools do"; the session-intent parser answers "what did the user ask". Its jq-based extraction is superseded here by `../scripts/session_intent.py` (stdlib, no jq dependency).
