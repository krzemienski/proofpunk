...
**End-user validation is inline after every task** — criteria first, detect
the platform, start the real runtime, YOU drive it, capture fresh run-scoped
evidence, verdict per criterion. The proof standard — Six Steps, Actor
Mandate, fresh-evidence sealing — is owned by `end-user-testing` and applied
verbatim here. The Iron Rule applies to every task: fix the real system,
never a mock; never write a test file as validation.

**Full protocol — the six validation steps, `--parallel` lane-contract
fields, escalation rules, and ledger semantics: `references/execution-loop.md`.
Load it when you enter Stage 5.**

## Stage 6 — The stuck protocol

Attempt the obvious fix once → root-cause loop (max 3 hypotheses) via
`root-cause-debugging` → split the task into smaller provable tasks →
escalate with a structured blocker report. Under `--auto`, rungs 1-3 never
stop the run; authorization boundaries always route to rung 4.
