> Incorporated from the `cpp-testing` skill (skills-ref.zip). **Adaptation:** This plugin enforces a no-mock Iron Rule: mock/stub chapters below are for UNDERSTANDING existing suites, not for writing new ones. New tests must run against the real system (real database, real server, real browser).
...
| NEVER | WHY | Fix |
|-------|-----|-----|
| Use `sleep()` for synchronization in concurrent tests | Wall-clock delays are non-deterministic; pass locally but flake in CI under load | Use `std::condition_variable`, `std::latch`, or `std::barrier` with bounded waits |
| Use fixed temp file paths across tests | Parallel test execution causes file conflicts; tests corrupt each other's state | Generate unique temp directories per test with `std::filesystem::temp_directory_path()` |
| Over-mock simple value objects | Mock overhead hides real behavior; tests become tightly coupled to implementation | Use real value objects; only mock for interaction verification on interfaces |
| Run tests without sanitizers in CI | Memory leaks, undefined behavior, and race conditions go undetected until production | Add ASan + UBSan builds in CI; add TSan for concurrent code |
